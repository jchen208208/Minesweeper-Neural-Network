...
If you get truly stuck, `plan.md` contains a full working version of every
file. Treat it like the answer key at the back of a textbook: check it
*after* you've made an honest attempt, not before.

**Read Part 1 of `plan.md` first** if you haven't — it explains all the
concepts (Q-values, replay buffer, target network, Double DQN, epsilon-greedy,
action masking). This manual assumes you've read it and focuses on the
*building*.
